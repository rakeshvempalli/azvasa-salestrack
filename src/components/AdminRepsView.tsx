...
export const AdminRepsView: React.FC<AdminRepsViewProps> = ({
  ...
  const handleToggleStatus = (user: UserProfile) => {
    ...
    if (user.role === 'super_admin' && superAdmins.length <= 1 && user.status === 'active') {
      setSuccessToast('Cannot deactivate the only active Super Admin account.');
      setTimeout(() => setSuccessToast(null), 4000);
      return;
    }
  ...
  const handleInitiateDelete = (user: UserProfile) => {
    ...
    if (user.role === 'super_admin' && superAdmins.length <= 1) {
      setSuccessToast('Cannot delete the only active Super Admin account.');
      setTimeout(() => setSuccessToast(null), 4000);
      return;
    }
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-xs text-[#646260] mt-1 max-w-2xl">
              Add corporate team members (Super Admin, Sales Manager, Sales Representatives). Configure their official company email, username, and password for direct system access.
            </p>
          </div>
          ...
          <table className="w-full text-left text-xs">
            ...
            <tbody className="divide-y divide-[#f3f2f1]">
              {filteredUsers.map(user => {
                ...
                const isSoleSuperAdmin = user.role === 'super_admin' && reps.filter(r => r.role === 'super_admin' && r.status === 'active').length <= 1;
                ...
                return (
                  <tr key={user.id} className="hover:bg-[#fafaf9] transition-colors">
                    ...
                    <td className="py-3.5 px-4">
                      <div className="flex items-center gap-1.5">
                        <Mail className="w-3.5 h-3.5 text-[#084ab8] shrink-0" />
                    ...
                    <td className="py-3.5 px-4">
                      <button
                        ...
                        title={isSoleSuperAdmin ? "The only active Super Admin cannot be deactivated" : "Toggle active status"}
                        ...
                        } ${isSoleSuperAdmin ? 'opacity-80 cursor-not-allowed' : ''}`}
                      >
        ...
        <div className="fixed inset-0 z-60 bg-black/40 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl max-w-lg w-full p-6 border border-[#e8e7e5] shadow-2xl space-y-4 max-h-[90vh] overflow-y-auto animate-scaleIn">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                <label className="block text-xs font-semibold text-[#2d2b2a] mb-1">
                  Company Email Address <span className="text-[#f28705]">*</span>
                </label>
                <input
                  type="email"
                  placeholder="e.g. employee@azvasa.com"
                  value={email}
                  onChange={(e) => {
                ...
                <span className="text-[10px] text-[#646260] mt-0.5 block">
                  Official corporate email address. The user can sign in using this email or username with password.
                </span>
              </div>
